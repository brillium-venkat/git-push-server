...
#   09-07-2022 - Updated for unification across OSes and pre-reqs
#
...
EXIT_MACOS_VERSION_CHECK_COMMAND_ERROR_CODE=121
...
appbrahma_pre_reqs_check() {
	...
	echo "$GENERATOR_LINE_PREFIX : Your MacOS version is : $(/usr/bin/sw_vers -productVersion)"
	...
	if [[ $(/usr/bin/sw_vers -productVersion | awk -F. '{ print $1 }') -ge $OS_MAJOR_VERSION ]]; then
		if [[ $(/usr/bin/sw_vers -productVersion | awk -F. '{ print $2 }') -ge $OS_MINOR_VERSION ]]; then
			if [[ $(/usr/bin/sw_vers -productVersion | awk -F. '{ print $3 }') -ge $OS_PATCH_VERSION ]]; then
				echo "$GENERATOR_LINE_PREFIX : MacOS version requirement - $OS_MAJOR_VERSION.$OS_MINOR_VERSION.$OS_PATCH_VERSION - met, moving ahead with other checks..."
			else
				echo "${RED}${BOLD}$GENERATOR_LINE_PREFIX : You are running unsupported MacOS version $(/usr/bin/sw_vers -productVersion) for building and running AppBrahma generated Unimobile application project sources!${NT}"
				echo "${RED}${BOLD}$GENERATOR_LINE_PREFIX : Minimum required version is $OS_MAJOR_VERSION.$OS_MINOR_VERSION.$OS_PATCH_VERSION${NT}"
				echo "${RED}${BOLD}$GENERATOR_LINE_PREFIX : Aborting the build process. Please retry after ensuring pre-requisites are met.${NT}"
...
if [[ ! "$docker_container_run_check" == "running" ]]; then
    ...
    if [ $? -gt 0 ]; then
        printf "${YELLOW}$GENERATOR_LINE_PREFIX : It appears docker daemon is not running. Attempting to launch the docker daemon...${NT}\n"
        open /Applications/Docker.app
        echo -n "${YELLOW}$GENERATOR_LINE_PREFIX : Waiting for docker daemon to complete the start-up..."
        sleep 1
        # Wait until docker daemon finishes starting up
        while (! docker stats --no-stream >/dev/null 2>&1); do
            echo -n "."
            sleep 1
        done
        echo "${NT}"
        echo "${GREEN}$GENERATOR_LINE_PREFIX : Docker daemon started up.${NT}"
    fi
    echo "${GREEN}$GENERATOR_LINE_PREFIX : Now spawning a command terminal for running configured database server in docker container...${NT}"
    dir="$(pwd)"
    run_db_in_docker=$(osascript <<EOF
        tell application "Terminal"
            do script " \
                clear && \
                tput setaf 2 && \
                tput bold && \
                echo \"$GENERATOR_LINE_PREFIX : Welcome to Docker DB Server for $GENERATOR_NAME\" && \
                tput sgr0 && \
                cd \"$dir\" && \
                docker-compose -f src/main/docker/mysql.yml up --remove-orphans \
            "
        end tell
    EOF)
    if [ $? -gt 0 ]; then
        echo "${RED}$GENERATOR_LINE_PREFIX : Error spawning command terminal or running configured db server in docker container!${NT}"
        echo "${RED}$GENERATOR_LINE_PREFIX : Error details are displayed below. Aborting the execution.${NT}"
        echo "$run_db_in_docker"
        echo "${RED}$GENERATOR_LINE_PREFIX : Please retry running the script after fixing above reported errors.${NT}"
        exit $EXIT_TERM_SPAWN_RUN_ERROR_CODE
    else
        echo "$GENERATOR_LINE_PREFIX : Spawned a command terminal for running configured database server in docker container. Will connect to this DB server. Proceeding ahead..."
    fi
else
    echo "$GENERATOR_LINE_PREFIX : Found docker container already running configured database server. Will connect to this DB server. Proceeding ahead..."
fi
...
if [ "$server_rest_api_mode" == "$HTTPS" ]; then
    ...
    read -p "${YELLOW}${BOLD}$GENERATOR_LINE_PREFIX : Press any key to continue...${NT}"
    dir="$(pwd)"
    spawn_and_run_backend_web_server=$(osascript <<EOF
        tell application "Terminal"
            do script " \
                clear && \
                tput setaf 2 && \
                tput bold && \
                echo \"$GENERATOR_LINE_PREFIX : Welcome to backend $GENERATOR_NAME Build and Run\" && \
                tput sgr0 && \
                cd \"$dir\" && \
                ./mvnw -Pdev,-webapp,tls \
            "
        end tell
    EOF)
    if [ $? -gt 0 ]; then
        ...
        ECHO "${RED}$GENERATOR_LINE_PREFIX : Error details are displayed below. Aborting the execution.${NT}"
        echo "$spawn_and_run_backend_web_server"
        echo "${RED}$GENERATOR_LINE_PREFIX : Please retry running the script after fixing above reported errors.${NT}"
        exit $EXIT_TERM_SPAWN_RUN_ERROR_CODE
    fi
    ...
    if [ "$webpack" == "$HMR" ]; then
        ...
        read -p "${YELLOW}${BOLD}$GENERATOR_LINE_PREFIX : Press any key to continue...${NT}"
        dir="$(pwd)"
        spawn_and_run_webpack_server=$(osascript <<EOF
            tell application "Terminal"
                do script " \
                    clear && \
                    tput setaf 2 && \
                    tput bold && \
                    echo \"$GENERATOR_LINE_PREFIX : Welcome to Webpack Live HMR Server for $GENERATOR_NAME\" && \
                    tput sgr0 && \
                    cd \"$dir\" && \
                    npm run start-tls --host=localhost --disable-host-check \
                "
            end tell
        EOF)
        if [ $? -gt 0 ]; then
    ...
    read -p "${YELLOW}${BOLD}$GENERATOR_LINE_PREFIX : Press any key to continue...${NT}"
    dir="$(pwd)"
    spawn_and_run_backend_web_server=$(
        osascript <<EOF
        tell application "Terminal"
            do script " \
                clear && \
    ...
    if [ "$webpack" == "$HMR" ]; then
        ...
        if [ $? -gt 0 ]; then
            echo "${RED}$GENERATOR_LINE_PREFIX : Error spawning command terminal or running webpack live hot module reload of front-end code changes!${NT}"
            echo "${RED}$GENERATOR_LINE_PREFIX : Error details are displayed below. Aborting the execution.${NT}"
            echo "$spawn_and_run_webpack_server"
            echo "${RED}$GENERATOR_LINE_PREFIX : Please retry running the script after fixing above reported errors.${NT}"
            exit $EXIT_TERM_SPAWN_RUN_ERROR_CODE
        fi
