#!/bin/bash
# ====================================================================================================================================
# mvn-apb-backend-server-build-and-run-script.sh
...
# echo "${GREEN}This red color font${NC}"
...
echo ""
printf "$${BIYellow}$GENERATOR_LINE_PREFIX : You typed - \"./apb-linux-server-build-n-run-mvn.sh $@\"${NC}\n"
echo ""
...
if [ $# -lt 3 ]; then
    ...
    echo "Usage:"
  	printf "    ${BIYellow}./apb-linux-server-build-n-run-mvn.sh | ${BIGreen}build-action | server-protocol | front-end-hot-reload-action${NC}\n" | column -t -x -s "|"
...
if [ "$server_rest_api_mode" == "$HTTPS" ]; then
    ...
    if [ "$ks_selection" == "Yes" ]; then
        ...
        echo ""
        read -p "${GREEN}${BOLD}$GENERATOR_LINE_PREFIX : Press any key to continue...${NC}"
    else
        ...
        read -p "${GREEN}${BOLD}$GENERATOR_LINE_PREFIX : Press any key to continue, when done..."
        echo ""
...
if [[ ! "$docker_container_run_check" == "running" ]]; then
    ...
    echo "$GENERATOR_LINE_PREFIX : Found UnifiedBuildScriptsTestMySQLServer docker container already running configured database server. Will connect to this DB server. Proceeding ahead."
fi
...
if [ "$server_rest_api_mode" == "$HTTPS" ]; then
    ...
    echo "$GENERATOR_LINE_PREFIX : Please wait for the back-end server to be up and running in the spawned terminal."
    echo ""
    read -p "${GREEN}${BOLD}$GENERATOR_LINE_PREFIX : Press any key when the above is done...${NC}"
    ...
    if [ "$webpack" == "$HMR" ]; then
        ...
        echo ""
        read -p "$GENERATOR_LINE_PREFIX : Press any key to continue..."
        ...
        echo ""
        read -p "$GENERATOR_LINE_PREFIX : Press any key to continue, after the above is done..."
    fi
fi
